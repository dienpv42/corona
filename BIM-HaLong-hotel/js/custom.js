...
$(function () {
  ...
  wind.on("scroll", function () {
    ...
    if (bodyScroll > 100) {
      ...
      logo.attr("src", "img/logo-bim-dark.png");
    } else {
      navbar.removeClass("nav-scroll");
      logo.attr("src", "img/logo-bim.png");
    }
  });
...
$('.gallery-container').each(function (index, container) {
  const $container = $(container);
  const $thumbs = $container.find('.thumbnail-container a');
  const $mainImage = $container.find('.main-image');
  const $mainLink = $container.find('.main-image-link');
  ...
  // Set ảnh đầu tiên
  const firstSrc = $thumbs.eq(0).attr('href');
  $mainImage.attr('src', firstSrc);
  $mainLink.attr('href', firstSrc);
  $thumbs.eq(0).addClass('active');

  // Hàm cập nhật ảnh lớn
  function updateMainImage(index) {
    const newSrc = $thumbs.eq(index).attr('href');
    $mainImage.attr('src', newSrc);
    $mainLink.attr('href', newSrc);
    $thumbs.removeClass('active');
    $thumbs.eq(index).addClass('active');
    $mainImage.stop(true, true).hide().fadeIn(300);
  }
  ...
  // Sự kiện click ảnh lớn mở Fancybox
  $mainLink.on('click', function (e) {
    e.preventDefault();

    // Tạo danh sách ảnh riêng biệt cho Fancybox gallery
    let images = [];
    $thumbs.each(function () {
      images.push({ src: $(this).attr('href'), type: 'image' });
    });
