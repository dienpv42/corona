...
$(".btn-edit").click(function(){
	...
	$.ajax({
		url: "{{route('post-edit-group-user')}}",
		type: 'POST',
		dataType: 'json',
		data: {id:$("#eid").val(),name:$("#ename").val(),description:$("#edescription").val()},	
		success: function (msg, textStatus, jqXHR) {
			if(msg.check==0){
...
function fill_datatable(filter_status = '') {
	var table = $('#partnerTable').DataTable({
		...
		ajax: {
			url: "{{route('post-data-group-user')}}",
			type: "POST",
			data: {
			...
			<div class="wrap_head">
				<div class="wrap_list_link_lb wrap_list_link_lb_bd">
					<ul class="list_link_lb">
						...
						<li class="list_link_item">
							<a href="{{route('view-group-user')}}" class="lb-xsmall lb-link active">
				...
				<div class="wrap_list_action wrap_list_action_bw">
					<a href="{{route('view-group-user')}}" class="lb-xsmall">
						<span>
