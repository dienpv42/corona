...
$(document).on('click', '.pedit', function() {
	...
	$.ajax({
		...
		success: function (msg, textStatus, jqXHR) {
			$("#ename").val(msg.name);
			tinymce.get('econtent').setContent(msg.content);
		}
	});
});
$(".btn-edit").click(function(){
	...
	$.ajax({
		url: "{{route('post-edit-email-template')}}",
		type: 'POST',
		dataType: 'json',
		data: {id:$("#eid").val(),name:$("#ename").val(),content:tinymce.get('econtent').getContent()},	
		success: function (msg, textStatus, jqXHR) {
			if(msg.check==0){
...
$(".btn-add").click(function(){
	...
	$.ajax({
		url: "{{route('post-add-email-template')}}",
		type: 'POST',
		dataType: 'json',
		data: {name:$("#name").val(),content:tinymce.get('content').getContent()},	
		success: function (msg, textStatus, jqXHR) {
			if(msg.check==0){
				toastr.error(msg.content, ''); 
			}else{				
...
function fill_datatable() {
	var table = $('#partnerTable').DataTable({
		...
		ajax: {
			url: "{{route('post-data-email-template')}}",
			type: "POST",
			data: {
		...
		"columns": [
			...
			},
			{
				...
				"render": function(data, type, row, meta) {					
					return '<a href="javascript:void(0)" data-id="'+data.id+'" data-bs-toggle="modal" data-bs-target="#modalEdit" class="lb-text-primary pedit">{{App\Models\CommonModel::get_lang("setting_product21")}}</a> | <a href="javascript:void(0)" data-url="'+data.url_delete+'" class="lb-text-primary premove">Xoá</a>'
            ...
            <div class="modal-body">
                ...
                <div class="wrap_form_modal">
                    ...
                    <div class="form-group">
                        ...
                        <div class="group">
                            <input type="text" class="form-control tinymce_editor" id="econtent">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="modal-body">
                <div class="wrap_form_modal">                    
					...
					<div class="form-group">
                        ...
                        <div class="group">
                            <input type="text" class="form-control tinymce_editor" id="content">
                        </div>
                    </div>
			...
			<div class="wrap_head">
				<div class="wrap_list_link_lb wrap_list_link_lb_bd">
					<ul class="list_link_lb">
						...
						<li class="list_link_item">
							<a href="{{route('view-email-template')}}" class="lb-xsmall lb-link active">
								Mẫu email
							</a>
						</li>
					</ul>
				...
				<div class="wrap_list_action wrap_list_action_bw">
					<a href="{{route('view-email-template')}}" class="lb-xsmall">
						<span>
