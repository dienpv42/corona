...
$(document).on('click', '.add_product', function() {
	if($("#product_id").val()){
		if($("#tr"+$("#product_id").val()).length === 0){
			var html="<tr id='tr"+$("#product_id").val()+"'><td><input type='hidden' class='list_id' value='"+$("#product_id").val()+"'/><input type='hidden' id='pricevat_"+$("#product_id").val()+"' class='list_vat' value='"+$("#product_id option:selected").data('vat')+"'/>"+$("#product_id option:selected").data('name')+"</td><td><input type='text' data-id='"+$("#product_id").val()+"' class='form-control list_price mask_number' value='"+$("#product_id option:selected").data('price')+"'/></td><td id='text_pricevat_"+$("#product_id").val()+"'>"+$("#product_id option:selected").data('pricevat')+"</td><td><input type='text' value='1' min='1' class='form-control list_quantity' onkeypress='return event.charCode >= 48 && event.charCode <= 57'/></td><td><a href='javascript:void(0)' class='remove_t'><i class='icofont-trash'></i></a></td></tr>";
...
$(".add_order").click(function(){
	...
	$.ajax({
		...
		type: 'POST',
		dataType: 'json',
		data: {list_id:list_id,list_price:list_price,list_vat:list_vat,list_quantity:list_quantity,id:$("#user_id").val()},	
		success: function (msg, textStatus, jqXHR) {
			if(msg.check==0){
				toastr.error(msg.content, ''); 
			}else{		
				window.location.replace("{{route('view-order')}}");
