...
$(document).on('click', '.pedit', function() {
	...
	$.ajax({
		...
		success: function (msg, textStatus, jqXHR) {
			$("#ename").val(msg.name);
			$("#eaddress").val(msg.address);
			$("#egroup_user_id").val(msg.group_user_id);
...
$(".btn-edit").click(function(){
	...
	$.ajax({
		...
		type: 'POST',
		dataType: 'json',
		data: {id:$("#eid").val(),name:$("#ename").val(),address:$("#eaddress").val(),group_user_id:$("#egroup_user_id").val(),phone:$("#ephone").val(),status:status},	
		success: function (msg, textStatus, jqXHR) {
			if(msg.check==0){
...
$(".btn-add").click(function(){
	...
	$.ajax({
		url: "{{route('post-add-user')}}",
		type: 'POST',
		dataType: 'json',
		data: {name:$("#name").val(),address:$("#address").val(),email:$("#email").val(),group_user_id:$("#group_user_id").val(),phone:$("#phone").val()},	
		success: function (msg, textStatus, jqXHR) {
			if(msg.check==0){
				toastr.error(msg.content, ''); 
			}else{				
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="wrap_form_modal">
					...
					<div class="form-group">
                        <div class="form-label">
                             {{App\Models\CommonModel::get_lang('setting_user3')}}
                        </div>
                        <div class="group">
                           <select class="form-control" id="egroup_user_id">
								@if(isset($group_user) && count($group_user)>0)
								@foreach($group_user as $item)
								<option value="{{$item->group_user_id}}">{{$item->name}}</option>
								@endforeach								
								@endif
							</select>
                        </div>
            ...
            <div class="modal-body">
                <div class="wrap_form_modal">
					...
					<div class="form-group">
                        ...
                        <div class="group">
                           <select class="form-control" id="group_user_id">
								@if(isset($group_user) && count($group_user)>0)
								@foreach($group_user as $item)
								<option value="{{$item->group_user_id}}">{{$item->name}}</option>
								@endforeach								
