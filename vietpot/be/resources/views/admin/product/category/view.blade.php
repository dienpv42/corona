...
$(".btn-edit").click(function(){
	...
	$.ajax({
		url: "{{route('post-edit-product-category')}}",
		type: 'POST',
		dataType: 'json',
		data: {id:$("#eid").val(),name:$("#ename").val()},	
		success: function (msg, textStatus, jqXHR) {
			if(msg.check==0){
...
$(".btn-add").click(function(){
	...
	$.ajax({
		url: "{{route('post-add-product-category')}}",
		type: 'POST',
		dataType: 'json',
...
function fill_datatable(filter_status = '') {
	var table = $('#partnerTable').DataTable({
		...
		ajax: {
			url: "{{route('post-data-product-category')}}",
			type: "POST",
			data: {
		...
		"columns": [{
			...
			},
			{
				...
				"render": function(data, type, row, meta) {					
					return '<a href="javascript:void(0)" class="pedit" data-id="'+data.id+'" data-bs-toggle="modal" data-bs-target="#modalEdit">Sửa</a> / <a href="javascript:void(0)" class="btn-action-table" data-url="'+data.url_delete+'">Xoá</a>'
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-footer justify-content-center">
                <button type="button" class="lb-xsmall-primary pd-s-100 pd-e-100 btn-edit">Lưu</button>
            </div>
        </div>
    </div>
</div>
			...
			<div class="wrap_head">
				<div class="wrap_list_link_lb wrap_list_link_lb_bd">
					<ul class="list_link_lb">
						...
						<li class="list_link_item">
							<a href="{{route('view-product-category')}}" class="lb-xsmall lb-link active">
				...
				<div class="wrap_list_action wrap_list_action_bw">
					<a href="{{route('view-product-category')}}" class="lb-xsmall">
						<span>
